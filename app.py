...
def afficher_situation(situation):
    mapping = {
        ...
        "Radié": "indisponible",
        "Défunt": "est décédé(e)",
        "Transféré": "a été transféré"
    }
    return mapping.get(situation, situation)

# --- Fonction d'affichage d'une archive (utilisée dans Diocèse et Paroisse) ---
def afficher_une_archive(a):
    """
    a est un tuple de 10 éléments :
    (matricule, nom, prenom, situation, date_debut, date_fin, commentaire,
     paroisse_nom, equipe_nom, auteur_nom)
    """
    matricule, nom, prenom, situation, date_debut_raw, date_fin_raw, commentaire, paroisse_nom, equipe_nom, auteur_nom = a
    ...
    icone = {"Déplacé": "📤", "Radié": "🚫", "Défunt": "🕊️", "Transféré": "🔄"}.get(situation, "📌")
    
    if situation == "Transféré":
        # Extraire la paroisse de destination depuis le commentaire
        match = re.search(r'vers (.+?) /', commentaire or "")
        nouvelle_paroisse = match.group(1) if match else "une autre paroisse"
        st.markdown(f"**{icone} {nom} {prenom} ({matricule})**")
        st.write(f"Désormais à **{nouvelle_paroisse}**")
        if date_debut_obj and date_fin_obj:
            st.write(f"a médité {duree} an(s) avec nous, d’sept {date_debut_obj.year} à sept {date_fin_obj.year}")
        if commentaire:
            st.write(f"📝 {commentaire}")
    else:
        situation_affichee = afficher_situation(situation)
        st.markdown(f"**{icone} {nom} {prenom} ({matricule}) – {situation_affichee}**")
        if date_debut_obj and date_fin_obj:
            st.write(f"Période : sept {date_debut_obj.year} – sept {date_fin_obj.year} ({duree} an(s))")
        if commentaire:
            st.write(f"📝 {commentaire}")
    st.write(f"✍️ Archivé par {auteur_nom}")
    st.markdown("---")

# --- Fonction de transfert (avec création d'archive et historique) ---
def transferer_membre(membre_id, nouvelle_paroisse_id, nouvelle_equipe_id, motif, auteur_id, auteur_nom, auteur_role):
    ...
    # Récupérer la date d'adhésion
    date_adhesion = c.execute("SELECT date_adhesion FROM membres WHERE id=?", (membre_id,)).fetchone()[0]
    if isinstance(date_adhesion, str):
        date_adhesion = date.fromisoformat(date_adhesion)
    date_transfert = date.today()
    
    # Récupérer les noms pour le commentaire
    nouvelle_paroisse_nom = c.execute("SELECT nom FROM paroisses WHERE id=?", (nouvelle_paroisse_id,)).fetchone()[0]
    nouvelle_equipe_nom = c.execute("SELECT nom_equipe FROM equipes WHERE id=?", (nouvelle_equipe_id,)).fetchone()[0]
    commentaire_archive = f"Transféré vers {nouvelle_paroisse_nom} / {nouvelle_equipe_nom}" + (f" - Motif : {motif}" if motif else "")
    
    # Vérification que l'équipe destination appartient à la paroisse destination
    ...
    # Insertion dans archives (situation 'Transféré')
    c.execute('''INSERT INTO archives (membre_id, situation, date_debut, date_fin, commentaire,
                 ...
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
              (membre_id, "Transféré", date_adhesion, date_transfert, commentaire_archive,
               auteur_id, auteur_nom, auteur_role, ancienne_paroisse, ancienne_equipe))
    
    # Enregistrement dans la table des mouvements (optionnel)
    c.execute('''INSERT INTO mouvements (membre_id, date_mouvement, ancienne_paroisse_id, nouvelle_paroisse_id,
                 ...
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
              (membre_id, date_transfert, ancienne_paroisse, nouvelle_paroisse_id,
...
def exporter_excel_diocese():
    ...
    try:
        ...
        st.error(f"Erreur lors de la création de l'Excel : {e}. Installez 'openpyxl' ou 'xlsxwriter'.")
        return None

# --- Création des tables (avec migrations) ---
...
try: c.execute("ALTER TABLE membres ADD COLUMN numero_equipe TEXT"); conn.commit()
except: pass
try: c.execute("ALTER TABLE membres ADD COLUMN numero_meditation TEXT"); conn.commit()
except: pass
...
if st.session_state['role'] == 'diocese':
    ...
    if menu == "Voir diocèse":
        ...
        if d:
            ...
            with st.expander("✏️ Modifier"):
                nouveau_respo = st.text_input("Nouveau responsable", value=d[0])
                nouveau_bureau = st.text_area("Nouveau bureau", value=d[1])
                if st.button("Enregistrer"):
                    c.execute("UPDATE diocese SET responsable=?, bureau=? WHERE id=?", (nouveau_respo, nouveau_bureau, 1))
                    conn.commit()
    ...
    # Gérer paroisses
        ...
        st.markdown('<h2 style="color:#1A237E; white-space: nowrap;">📋 Consultation des paroisses</h2>', unsafe_allow_html=True)
    ...
    # Rechercher par matricule
        ...
        if matricule:
            m = c.execute('''SELECT m.matricule, m.nom, m.prenom, m.whatsapp, p.nom, e.nom_equipe, m.photo_path
                             ...
                             JOIN paroisses p ON m.paroisse_id = p.id
                             JOIN equipes e ON m.equipe_id = e.id
                             WHERE m.matricule = ? AND m.statut = ?''', (matricule.upper(), 'actif')).fetchone()
            if m:
...
# ==================== PAROISSE ====================
    ...
    menu = st.sidebar.radio("Navigation", ["Ma paroisse", "Mes équipes", "Membres", "Statistiques", "Abonnements", "💬 WhatsApp", "📦 Archives"])
    ...
    # Mes équipes (Paroisse) - Version fluidifiée
        ...
        if not equipes:
            ...
            for eq in equipes:
                ...
                with st.expander(f"📌 {eq_nom} - Resp: {eq_resp} ({nb_membres}/10 membres)"):
                    st.write(f"**Bureau :** {eq_bureau}")
    ...
    # Statistiques
        ...
        nb_m = c.execute("SELECT COUNT(*) FROM membres WHERE paroisse_id=? AND statut=?", (pid, 'actif')).fetchone()[0]
        col1, col2 = st.columns(2)
    ...
    # Export Excel
        ...
        membres = c.execute('''SELECT m.matricule, m.nom, m.prenom, m.date_naissance, m.whatsapp, m.date_adhesion, e.nom_equipe
                               FROM membres m
                               JOIN equipes e ON m.equipe_id = e.id
                               WHERE m.paroisse_id = ? AND m.statut=?
                               ORDER BY e.nom_equipe, m.nom''', (pid, 'actif')).fetchall()
        if membres:
    ...
    # Archives (Paroisse)
        ...
        st.markdown(f'<h2 style="color:#1A237E;">📦 Archives de {nom_paroisse}</h2>', unsafe_allow_html=True)
        ...
        if not archives:
            ...
            for a in archives:
                matricule, nom, prenom, situation, date_debut_raw, date_fin_raw, commentaire, equipe_nom, auteur_nom = a
                date_debut_obj = convertir_en_date(date_debut_raw)
                date_fin_obj = convertir_en_date(date_fin_raw)
                duree = (date_fin_obj - date_debut_obj).days // 365 if (date_debut_obj and date_fin_obj) else 0
                icone = {"Déplacé": "📤", "Radié": "🚫", "Défunt": "🕊️", "Transféré": "🔄"}.get(situation, "📌")
                ...
                if situation == "Transféré":
                    match = re.search(r'vers (.+?) /', commentaire or "")
                    nouvelle_paroisse = match.group(1) if match else "une autre paroisse"
                    with st.expander(f"{icone} {nom} {prenom} ({matricule}) – {situation_affichee}"):
                        st.write(f"Désormais à **{nouvelle_paroisse}**")
                        if date_debut_obj and date_fin_obj:
                            st.write(f"a médité {duree} an(s) avec nous, de sept {date_debut_obj.year} à sept {date_fin_obj.year}")
                        if commentaire:
                            st.write(f"📝 {commentaire}")
                        st.write(f"✍️ Archivé par {auteur_nom}")
                else:
                    with st.expander(f"{icone} {nom} {prenom} ({matricule}) – {situation_affichee}"):
                        ...
                        if commentaire:
                            st.write(f"📝 {commentaire}")
                        st.write(f"✍️ Archivé par {auteur_nom}")
                        st.write(f"Équipe : {equipe_nom or 'Inconnue'}")
...
# ==================== ÉQUIPE ====================
    ...
    # Abonnements (Équipe) - Version fluide
    elif menu == "Abonnements":
        ...
        # Statistiques de l'équipe
        total_membres = c.execute("SELECT COUNT(*) FROM membres WHERE equipe_id=? AND statut='actif'", (eid,)).fetchone()[0]
        payes = c.execute("SELECT COUNT(*) FROM abonnements WHERE membre_id IN (SELECT id FROM membres WHERE equipe_id=?) AND annee_debut=? AND statut='paye'", (eid, annee_debut)).fetchone()[0]
        
        col1, col2 = st.columns(2)
        col1.metric("📊 Total membres", total_membres)
        col2.metric("✅ Abonnements", payes, delta=f"{payes/total_membres*100:.0f}%" if total_membres else "0%")
        
        st.markdown("---")
        
        # Liste des membres avec gestion des abonnements
        ...
        if not membres:
            ...
            abonnes = c.execute("""
                SELECT m.nom, m.prenom, m.matricule, a.date_paiement, a.montant
                FROM membres m
            ...
            non_inscrits = c.execute("""
                SELECT m.nom, m.prenom, m.matricule
                FROM membres m
                WHERE m.equipe_id=? AND m.statut='actif' 
                AND m.id NOT IN (SELECT a.membre_id FROM abonnements a WHERE a.annee_debut=? AND a.statut='paye')
                ORDER BY m.nom
            """, (eid, annee_debut)).fetchall()
            ...
            with tab1:
                st.write(f"**Période :** {periode_aff}")
                if abonnes:
                    data = [{"Nom": a[0], "Prénom": a[1], "Matricule": a[2], "Date paiement": a[3], "Montant": f"{a[4]} FCFA"} for a in abonnes]
                    st.dataframe(pd.DataFrame(data), use_container_width=True)
                    output = io.BytesIO()
                    try:
                        with pd.ExcelWriter(output, engine=None) as writer:
                            pd.DataFrame(data).to_excel(writer, sheet_name="Abonnes", index=False)
                        output.seek(0)
                        st.download_button("📥 Exporter les abonnés", data=output,
                                        file_name=f"abonnes_{nom_equipe}_{paroisse_nom}_{annee_debut}.xlsx",
                                        key=f"export_abos_eq_{eid}_{annee_debut}")
                    except Exception as e:
                        st.error(f"Erreur export: {e}")
                else:
            ...
            with tab2:
                st.write(f"**Période :** {periode_aff}")
                if reabonnes:
                    data = [{"Nom": r[0], "Prénom": r[1], "Matricule": r[2], "Date paiement": r[3], "Montant": f"{r[4]} FCFA"} for r in reabonnes]
                    st.dataframe(pd.DataFrame(data), use_container_width=True)
                    output = io.BytesIO()
                    try:
                        with pd.ExcelWriter(output, engine=None) as writer:
                            pd.DataFrame(data).to_excel(writer, sheet_name="Reabonnes", index=False)
                        output.seek(0)
                        st.download_button("📥 Exporter les réabonnés", data=output,
                                        file_name=f"reabonnes_{nom_equipe}_{paroisse_nom}_{annee_debut}.xlsx",
                                        key=f"export_reabos_eq_{eid}_{annee_debut}")
                    except Exception as e:
                        st.error(f"Erreur export: {e}")
                else:
    ...
    # Archives (avec gestion complète)
        ...
        membres_actifs = c.execute("SELECT id, nom, prenom, matricule FROM membres WHERE equipe_id=? AND statut=? ORDER BY nom", (eid, 'actif')).fetchall()
        ...
        # Gestion des archives existantes (affichage avec édition)
        if archives_equipe:
            ...
            for arch in archives_equipe:
                ...
                situation_affichee = afficher_situation(situation)
                icone = {"Déplacé":"📤","Radié":"🚫","Défunt":"🕊️","Transféré":"🔄"}.get(situation,"📌")
                with st.expander(f"{icone} {nom} {prenom} ({matricule}) – {situation_affichee} – a médité {duree} an(s) avec nous - de sept {annee_debut_aff} à sept {annee_fin_aff}"):
